...
SDKs and protocols — **DJI Cloud API (REST + MQTT)**, **MAVLink** (via
MAVSDK-Python, no `mavros` required), **Parrot Olympe**, etc. None of those
require ROS2.
...
- **MAVLink is adopted directly** (MAVSDK-Python) — covers a huge slice of the
  non-DJI market without dragging in ROS2.
...
- **MAVLink via `mavros` inside ROS2** — rejected: pulls all of ROS2 in for no
  added capability over MAVSDK-Python.
